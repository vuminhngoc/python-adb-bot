...
def find_and_take(limit_x, limit_y):
    ...
    try:
        last_mine = screenshot_image[star_x + 15:star_x + 15 + 95, star_y - 8: star_y - 8 + 14]
        last_lvl = screenshot_image[star_x - 15:star_x - 15 + 170, star_y - 44: star_y - 44 + 27]
    except:
        logging.debug("Không thấy start button, hoặc lỗi chụp")
    ...
    try:
        try:
            ...
            if mine_level > share_lv:
                last_mine_screenshot = pyautogui.screenshot("imgs/last_mine_screenshot.png",
                                                            region=(star_x - 70, star_y - 50, 250, 420))
        ...
        if is_full_queue or share_status or mine_level <= share_lv:
            _x, _y = pyautogui.locateCenterOnScreen("imgs/share.png", grayscale=True, confidence=0.85)
            ...
            for i in range(10):
                sleep(0.5)
                try:
                    _x, _y = pyautogui.locateCenterOnScreen("imgs/alliance.png", grayscale=True, confidence=0.85)
                    pyautogui.moveTo(_x, _y, duration=0.4)
                    pyautogui.click()
            ...
            sleep(0.2)
            _x, _y = pyautogui.locateCenterOnScreen("imgs/share_button.png", grayscale=True, confidence=0.85)
            pyautogui.moveTo(_x, _y, duration=0.4)
            pyautogui.click()
            logging.debug('press Share button')

        _x, _y = pyautogui.locateCenterOnScreen("imgs/back.png", grayscale=True, confidence=0.85)
        pyautogui.moveTo(_x, _y, duration=0.4)
        pyautogui.click()
        logging.debug('press Back')
    except:
...
if __name__ == '__main__':
    ...
    # imgx = device.screencap()
    # screenshot_image = cv2.imdecode(np.frombuffer(imgx, np.uint8), -1)
    # cv2.imshow("Sheep", screenshot_image)
    # cv2.waitKey(0)
    # sleep(1)
    imgx = device.screencap()
    screenshot_image = cv2.imdecode(np.frombuffer(imgx, np.uint8), -1)
    x = screenshot_image[500:850, 400:750]


    # img = cv2.imread("imgs/s3.jpg", cv2.IMREAD_ANYCOLOR)
    #
